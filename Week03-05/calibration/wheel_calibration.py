...
def calibrateWheelRadius(): # linear velocity
    ...
    for delta_time, wheel_vel in zip(delta_times, wheel_velocities_range):
        ...
        dist_robot = delta_time*wheel_vel 
        scale += 1/dist_robot
    scale = scale/num
...
def calibrateBaseline(scale): # angular velocity
    ...
    for delta_time, wheel_vel in zip(delta_times, wheel_velocities_range):
        ...
        radius = delta_time*wheel_vel/(2*np.pi)*scale
        baseline += 2*radius
