...
class Robot:
    ...
    def derivative_drive(self, drive_meas):
        ...
        if ang_vel == 0:
            ...
            R = lin_vel/ang_vel
            DFx[0,2] = R*(np.cos(th+ang_vel*dt)-np.cos(th))
            DFx[1,2] = R*(np.sin(th+ang_vel*dt)-np.sin(th))
    ...
    def covariance_drive(self, drive_meas):
        ...
        if ang_vel == 0:
            ...
            Jac2[0,1] = lin_vel*(dt*np.cos(th2)/ang_vel + (np.sin(th)-np.sin(th2))/(ang_vel**2))
            Jac2[1,0] = (-np.cos(th2) + np.cos(th)) / ang_vel
            Jac2[1,1] = lin_vel*(dt*np.sin(th2)/ang_vel + (np.cos(th2)-np.cos(th))/(ang_vel**2))
            Jac2[2,1] = dt 
